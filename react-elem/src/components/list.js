...
class ListItem extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      ...
      text: this.props.data.delivery_mode
        ? this.props.data.delivery_mode.text
        : "",
  ...
  handleClick() {
    /*用户的地址hash和商家id*/
    // window.location.href = `/shop/${this.props.address}/${
    //   this.state.restaurant_id
    // }`;
    console.log(this.props, "props");
    this.props.history.replace(
      `/shop/${this.props.address}/${this.state.restaurant_id}`
    );
  }
  /*图片格式化*/
  _formatImg() {
              ...
              <h3
                className={
                  this.state.is_premium
                    ? "is_premium  show_line_h3"
                    : "show_line_h3"
                }
              >
                <span>{this.state.name}</span>
              </h3>
              <ul className="show_supportWrap">{this.state.supportsDOM}</ul>
            </section>
            <section className="show_line show_line2">
              <div className="show_rateWrap">
                <div>
                  <div
                    className="show_star"
                    style={{ width: `${(100 * this.state.rating) / 5}%` }}
                  >
                    <svg
                ...
                <span className="show_num">
                  月售{this.state.recent_order_num}单
                </span>
              </div>
              {this.state.is_solid ? (
